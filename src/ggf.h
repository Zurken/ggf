...
#define GGF_ASSERT(expression)                                                 \
  {                                                                            \
    if (!(expression)) {                                                       \
      GGF_FATAL("%s:%d ASSERTION FAILED - '%s'", __FILE__, __LINE__,           \
                #expression);                                                  \
      __builtin_trap();                                                        \
    }                                                                          \
  }
...
#define GGF_INFO(message, ...)                                                 \
  ggf_platform_console_write(GGF_PLATFORM_CONSOLE_COLOR_GREEN, message,        \
                             ##__VA_ARGS__)
#define GGF_WARN(message, ...)                                                 \
  ggf_platform_console_write(GGF_PLATFORM_CONSOLE_COLOR_YELLOW, message,       \
                             ##__VA_ARGS__)
#define GGF_DEBUG(message, ...)                                                \
  ggf_platform_console_write(GGF_PLATFORM_CONSOLE_COLOR_BLUE, message,         \
                             ##__VA_ARGS__)
#define GGF_ERROR(message, ...)                                                \
  ggf_platform_console_write_error(GGF_PLATFORM_CONSOLE_COLOR_RED, message,    \
                                   ##__VA_ARGS__)
#define GGF_FATAL(message, ...)                                                \
  ggf_platform_console_write_error(GGF_PLATFORM_CONSOLE_COLOR_RED + 10,        \
                                   message, ##__VA_ARGS__)
...
b32 ggf_file_read_line(ggf_file_handle_t handle, u64 max_line_length,
...
// returns TRUE if the provided key was just pressed. key is a GGF_KEY_*
// constant
b32 ggf_input_key_pressed(ggf_key_t key);
// returns TRUE if the provided key was just released. key is a GGF_KEY_*
// constant
b32 ggf_input_key_released(ggf_key_t key);

// returns TRUE if the provided mouse button is held down. button is a
// GGF_MOUSE_BUTTON* constant
b32 ggf_input_mouse_down(ggf_mouse_button_t button);
// returns TRUE if the provided mouse button was just pressed. button is a
// GGF_MOUSE_BUTTON_* constant
b32 ggf_input_mouse_pressed(ggf_mouse_button_t button);
// returns TRUE if the provided mouse button was just released. button is a
// GGF_MOUSE_BUTTON_* constant
...
b32 ggf_freelist_create(u64 size, u64 *memory_requirement, void *memory,
...
static inline ggf_hash_map_iter_t ggf_hash_map_begin(ggf_hash_map_t *map) {
  return map->buckets;
}
static inline ggf_hash_map_iter_t ggf_hash_map_end(ggf_hash_map_t *map) {
  return map->buckets +
         map->buckets_reserved_count * (map->key_size + map->value_size);
}
ggf_hash_map_iter_t ggf_hash_map_next(ggf_hash_map_t *map,
                                      ggf_hash_map_iter_t current);
static inline void *ggf_hash_map_key_from_iter(ggf_hash_map_t *map,
                                               ggf_hash_map_iter_t iter) {
  return (void *)iter;
}
static inline void *ggf_hash_map_value_from_iter(ggf_hash_map_t *map,
                                                 ggf_hash_map_iter_t iter) {
  return (void *)(iter + map->key_size);
}
...
#define GGF_HM_CREATE(key_type, value_type, empty_key, key_cmp_func,           \
                      hash_func, out_map)                                      \
  {                                                                            \
    key_type ek = empty_key;                                                   \
    u64 requirement = 0;                                                       \
    ggf_hash_map_create(512, sizeof(key_type), sizeof(value_type), 0, 0, 0, 0, \
                        &requirement, 0);                                      \
    void *mem = ggf_memory_alloc(requirement, GGF_MEMORY_TAG_HASH_MAP);        \
    ggf_hash_map_create(512, sizeof(key_type), sizeof(value_type), &ek,        \
                        &key_cmp_func, &hash_func, mem, &requirement,          \
                        &out_map);                                             \
  }
...
#define GGF_HM_GET(map, key, out)                                              \
  {                                                                            \
    __typeof(key) k = key;                                                     \
    out = (__typeof(out) *)ggf_hash_map_value_from_iter(                       \
        &map, ggf_hash_map_find(&map, &k));                                    \
  }
...
/*
    STAGE:
        ...
        when creating one you provide the name of all the assets and their
...
b32 ggf_asset_system_find_full_asset_path(ggf_asset_type_t type,
                                          const char *filename,
                                          u64 out_buffer_len, char *out_buffer);
...
// initialize graphics, assumes a valid OpenGL context is in use - returns TRUE
// if successful
...
// draw a triangle
void ggf_gfx_draw_triangle(vec2 p1, vec2 p2, vec2 p3, f32 depth, vec4 color,
...
void ggf_draw_quad_extent(vec2 pos, vec2 size, f32 depth, vec4 color,
                          ggf_texture_t *texture);
void ggf_gfx_draw_quad_corners(vec2 tl, vec2 br, f32 depth, vec4 color,
                               ggf_texture_t *texture) {
  ggf_gfx_draw_quad(tl, (vec2){br[0], tl[1]}, br, (vec2){tl[0], br[1]}, depth,
                    color, texture);
}
